...
- [x] Type B 服務已接入：
  - `ViewerService`（prismarine-viewer）— `bot.viewer`
  - `WebInventoryService`（mineflayer-web-inventory）— `bot.inventory_viewer`
- [ ] 已知痛點：事件 payload 與 mineflayer docs 尚未全面對齊；raw escape hatch 邊界仍偏寬；npm 插件版本未釘選（JSPyBridge lazy install 會拉 latest）
- [ ] 待 spike：gui（comparator 橋接）、statemachine（tick-driven callback）、dashboard（blessed terminal）、panorama（native 依賴）
...
2026-04-11 — Type B service 的 start() 必須走 done-event 模式回報成功/失敗，不可在呼叫後立即標為 started — 防止 port 衝突等非同步錯誤被隱藏
