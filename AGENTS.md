...
3. `@On` callback 的參數語意與 JSPyBridge / Node 版本有關：舊版 patched emitter 可能會把 emitter/`this` 放在第一個參數；目前 Node 18+ 目標環境通常直接收到 event payload。bridge 層必須依實際 runtime 正規化，不能硬編碼假設第一個參數一定是 `this`
