...
不是所有 mineflayer 插件都走同一種模式。依原始碼分三類：

- **Type A（`bot.loadPlugin()` facade）**：export 一個 `(bot) => void` 函式，透過 `bot.loadPlugin()` 載入。包括 armor-manager、tool、collectblock、gui、hawkeye。走 `PluginRegistry` 管理。
- **Type B（Server / Service）**：直接呼叫 `module(bot, options)` 初始化，不走 `bot.loadPlugin()`。包括 prismarine-viewer、web-inventory、panorama。透過 `bot.<service>` lazy property 管理。
...
能 await JS Promise 的就直接 await，不要自造 `_minethon:*Done` 事件。
...
- [x] 已知痛點：事件 payload 與 mineflayer docs 尚未全面對齊；同步 property 尚未完全 snapshot 化；raw escape hatch 邊界仍偏寬；插件系統目前實際只有 pathfinder 的初步接入
...
2026-04-11 — 插件分三類整合模式（Type A/B/C），不硬套同一抽象 — 原始碼驗證各插件 export pattern 不同
...
2026-04-11 — 能 await JS Promise 的直接 await，不自造 Done event — JSPyBridge bridge.js 已支援 Promise await
