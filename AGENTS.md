...
- [x] 已知痛點：事件 payload 與 mineflayer docs 尚未全面對齊；同步 property 尚未完全 snapshot 化（`is_alive`、`username` 首次存取仍做 live bridge I/O）；raw escape hatch 邊界仍偏寬；插件系統目前實際只有 pathfinder 的初步接入；npm 插件版本未釘選（JSPyBridge lazy install 會拉 latest）
