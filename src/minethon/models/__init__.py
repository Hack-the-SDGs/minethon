"""Runtime-importable type shells for `minethon.models`.

These empty placeholder classes exist so users can write annotations like
``from minethon.models import ChatMessage`` without hitting ImportError.
Their real member surfaces live in `src/minethon/bot.pyi` (the type stub).

Hover, completion, and `isinstance`-style work all key off the `.pyi`; at
runtime these placeholders are never the actual instance type — `bot.entity`
returns a JS proxy from JSPyBridge.
"""
