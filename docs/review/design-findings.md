...
### 修完之後還剩的一個安靜路徑（誠實記錄，沒修）

`dig()` 在「前面根本沒有東西」時仍然安靜回傳 `None`。實機跑 q01_swim 的形狀：

```
我在 (337.28, 63.0, -475.40)
（三次 bot.dig() 完全沒有輸出）
結束，我在 (334.11, 63.0, -471.06)
```

這是**原本就有的契約**（docstring 寫「沒有東西可挖就回 `None`」），而且是對的：
平視之後對著空地挖，本來就沒有東西。在會重複挖空氣的迴圈裡加提示會變成洗頻。
但學員的體驗仍然是「機器人走了三格、挖了三次、什麼都沒發生、也沒有任何字」。
若要處理，方向是讓 `dig()` 區分「沒有目標」和「有目標但沒破壞」——前者印
一次就好（例如只在連續第一次時提示）。**這一條我刻意沒動，因為它是設計取捨
而不是 bug，決定權在你。**

`look_level()` 的副作用要注意：修好 P0-2 之後，對著空地的 `dig()` 從「挖到腳下地板」
變成「什麼都不挖」。前者是錯的但看得到動作，後者是對的但完全安靜。
關卡裡目標在正前方時兩者都不會出現。
